class ADFRC_13Rnd_9x19_Mag : 16Rnd_9x21_Mag {
			ammo = "ADFRC_9x19_124_FMJ";
			count = 13;
			descriptionshort = "Caliber: 9MM <br />Rounds: 13<br />Used in: BHP";
			modelSpecial = "ADF_Weapons\adfrc_HPIII\Magazine\HPIII_Mag.p3d";
			modelSpecialIsProxy = 1;
			displayname = "Browning HP 13rd 9mm NATO Ball";
			lastroundstracer = 0;
			picture = "ADF_Weapons\adfrc_g19\data\ui\Gear_ADFRC_G19_Mag.paa";
			scope = 2;
			tracersevery = 0;
			mass = 2.75;
			displaynameshort = "FMJ";
			author = "Brucey";
		};
		
class ADFRC_13Rnd_9x19_FRANGE_Mag : ADFRC_13Rnd_9x19_Mag {
			ammo = "ADFRC_9x19_98_FRANG";
			displayname = "Browning HP 13rd 9mm FRANGABLE (Training)";
			mass = 2.6;
			displaynameshort = "FRANG";
		};